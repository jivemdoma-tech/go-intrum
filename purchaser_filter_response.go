package gointrum

import (
	"encoding/json"
	"strconv"
	"strings"
	"time"
)

type PurchaserFilterResponse struct {
	*Response
	Data *PurchaserFilterData `json:"data,omitempty"`
}

type PurchaserFilterData struct {
	List  []Purchaser `json:"list"`
	Count bool        `json:"count"`
}

type Purchaser struct {
	ID                   int64                     `json:"id,string"`
	GroupID              string                    `json:"group_id"`
	Name                 string                    `json:"name"`
	Surname              string                    `json:"surname"`
	Secondname           string                    `json:"secondname"`
	ManagerID            int64                     `json:"manager_id,string"`
	Phone                []*Phone                  `json:"phone"`
	Address              string                    `json:"address"`
	CreateDate           time.Time                 `json:"create_date"`
	Marktype             int64                     `json:"marktype,string"`
	Comment              string                    `json:"comment"`
	CustomerActivityType string                    `json:"customer_activity_type"`
	CustomerActivityDate time.Time                 `json:"customer_activity_date"`
	CustomerCreatorID    int64                     `json:"customer_creator_id,string"`
	Fields               map[int64]*PurchaserField `json:"fields"`
	EmployeeID           int64                     `json:"employee_id,string"`
	AdditionalManagerID  []int64                   `json:"additional_manager_id"`
	AdditionalEmployeeID []int64                   `json:"additional_employee_id"`
	// Markname             string                     `json:"markname"`
	// Nattype              string                     `json:"nattype"`
	// Email                []interface{}              `json:"email"`

}

type PurchaserField struct {
	ID       int64  `json:"id,string"`
	Datatype string `json:"datatype"`
	Value    any    `json:"value"`
}

type Phone struct {
	Phone   string `json:"phone"`
	Comment string `json:"comment"`
}

func (p *Purchaser) UnmarshalJSON(data []byte) error {
	// Оригинальная структура типа Alias для предовтращения рекурсии
	type Alias Purchaser

	// Вспомогательная структура
	var aux = &struct {
		*Alias
		CreateDate           string   `json:"create_date"`
		CustomerActivityDate string   `json:"customer_activity_date"`
		AdditionalManagerID  []string `json:"additional_manager_id"`
		AdditionalEmployeeID []string `json:"additional_employee_id"`
		Fields               any      `json:"fields"`
	}{
		Alias: (*Alias)(p), // Приведение типа к Alias
	}
	// Декодирование JSON во вспомогательную структуру
	if err := json.Unmarshal(data, &aux); err != nil {
		return err
	}

	// Замена

	parsedDate, err := time.Parse(DatetimeLayout, aux.CreateDate)
	if err != nil {
		return err
	}
	p.CreateDate = parsedDate

	parsedDate, err = time.Parse(DatetimeLayout, aux.CustomerActivityDate)
	if err != nil {
		return err
	}
	p.CustomerActivityDate = parsedDate

	// Замена массивов

	newSlice := make([]int64, 0, len(aux.AdditionalManagerID))
	for _, v := range aux.AdditionalManagerID {
		if value, err := strconv.ParseInt(v, 10, 64); err == nil {
			newSlice = append(newSlice, value)
		}
	}
	p.AdditionalManagerID = newSlice

	newSlice = make([]int64, 0, len(aux.AdditionalEmployeeID))
	for _, v := range aux.AdditionalEmployeeID {
		if value, err := strconv.ParseInt(v, 10, 64); err == nil {
			newSlice = append(newSlice, value)
		}
	}
	p.AdditionalEmployeeID = newSlice

	fieldsMap := make(map[int64]*PurchaserField)
	fieldsRaw, ok := aux.Fields.(map[string]any)
	if ok {
		for k, v := range fieldsRaw {
			id, err := strconv.ParseInt(k, 10, 64)
			if err != nil {
				continue // или логировать
			}
			valueBytes, _ := json.Marshal(v)
			var field PurchaserField
			if err := json.Unmarshal(valueBytes, &field); err != nil {
				continue // или логировать
			}
			fieldsMap[id] = &field
		}
	}
	p.Fields = fieldsMap

	return nil
}

// Методы получения значений Purchaser

// Вспомогательная функция получения структуры поля
func (p *Purchaser) getField(fieldID int64) (*PurchaserField, bool) {
	f, exists := p.Fields[fieldID]
	return f, exists
}

func (p *Purchaser) getFieldMap(fieldID int64) (map[string]string, bool) {
	f, exists := p.getField(fieldID)
	if !exists {
		return nil, false
	}
	m, ok := f.Value.(map[string]string)
	if !ok {
		return nil, false
	}
	return m, true
}

// text
func (p *Purchaser) GetFieldText(fieldID int64) string {
	f, exists := p.getField(fieldID)
	if !exists {
		return ""
	}
	vStr, ok := f.Value.(string)
	if !ok {
		return ""
	}
	return vStr
}

// radio
func (p *Purchaser) GetFieldRadio(fieldID int64) bool {
	vStr := p.GetFieldText(fieldID)
	if v, err := strconv.ParseBool(vStr); err == nil {
		return v
	}
	return false
}

// select
func (p *Purchaser) GetFieldSelect(fieldID int64) string {
	return p.GetFieldText(fieldID)
}

// multiselect
func (p *Purchaser) GetFieldMultiselect(fieldID int64) []string {
	return strings.Split(p.GetFieldText(fieldID), ",")
}

// date
func (p *Purchaser) GetFieldDate(fieldID int64) time.Time {
	vStr := p.GetFieldText(fieldID)
	return parseTime(vStr, DateLayout)
}

// datetime
func (p *Purchaser) GetFieldDatetime(fieldID int64) time.Time {
	vStr := p.GetFieldText(fieldID)
	return parseTime(vStr, DatetimeLayout)
}

// time
func (p *Purchaser) GetFieldTime(fieldID int64) time.Time {
	vStr := p.GetFieldText(fieldID)
	return parseTime(vStr, TimeLayout)
}

// integer
func (p *Purchaser) GetFieldInteger(fieldID int64) int64 {
	vStr := p.GetFieldText(fieldID)
	return parseInt(vStr)
}

// decimal
func (p *Purchaser) GetFieldDecimal(fieldID int64) float64 {
	vStr := p.GetFieldText(fieldID)
	return parseFloat(vStr)
}

// price
func (p *Purchaser) GetFieldPrice(fieldID int64) float64 {
	vStr := p.GetFieldText(fieldID)
	return parseFloat(vStr)
}

// file
func (p *Purchaser) GetFieldFile(fieldID int64) string {
	return p.GetFieldText(fieldID)
}

// point
func (p *Purchaser) GetFieldPoint(fieldID int64) [2]string {
	m, ok := p.getFieldMap(fieldID)
	if !ok {
		return [2]string{}
	}
	return [2]string{m["x"], m["y"]}
}

// integer_range
func (p *Purchaser) GetFieldIntegerRange(fieldID int64) [2]int64 {
	m, ok := p.getFieldMap(fieldID)
	if !ok {
		return [2]int64{}
	}
	return parseRange(m, parseInt)
}

// decimal_range
func (p *Purchaser) GetFieldDecimalRange(fieldID int64) [2]float64 {
	m, ok := p.getFieldMap(fieldID)
	if !ok {
		return [2]float64{}
	}
	return parseRange(m, parseFloat)
}

// date_range
func (p *Purchaser) GetFieldDateRange(fieldID int64) [2]time.Time {
	m, ok := p.getFieldMap(fieldID)
	if !ok {
		return [2]time.Time{}
	}
	return parseRange(m, func(p string) time.Time {
		return parseTime(p, DateLayout)
	})
}

// time_range
func (p *Purchaser) GetFieldTimeRange(fieldID int64) [2]time.Time {
	m, ok := p.getFieldMap(fieldID)
	if !ok {
		return [2]time.Time{}
	}
	return parseRange(m, func(p string) time.Time {
		return parseTime(p, DateLayout)
	})
}

// datetime_range
func (p *Purchaser) GetFieldDatetimeRange(fieldID int64) [2]time.Time {
	m, ok := p.getFieldMap(fieldID)
	if !ok {
		return [2]time.Time{}
	}
	return parseRange(m, func(p string) time.Time {
		return parseTime(p, DateLayout)
	})
}

// attach
func (p *Purchaser) GetFieldAttach(fieldID int64) []int64 {
	f, exists := p.getField(fieldID)
	if !exists {
		return nil
	}
	vAttach, ok := f.Value.([]map[string]string)
	if !ok || len(vAttach) <= 0 {
		return nil
	}
	vIDs := make([]int64, 0, len(vAttach))
	for _, v := range vAttach {
		if id, err := strconv.ParseInt(v["id"], 10, 64); err == nil {
			vIDs = append(vIDs, id)
		}
	}
	return vIDs
}
